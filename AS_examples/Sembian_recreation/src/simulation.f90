...
contains
   ...
   subroutine simulation_init
      ...
      create_and_initialize_flow_solver: block
         ...
         real(WP) :: EWleft,EWright ! left and right end points of high pressure region
         ...
         !AS change init from here to setup Sembian EW case
         ! we need a high pressure region init just after the start of the domain
         ! do not initialize a shock as we do normally
         ! this will be an unsteady shock and there will not be a steady inflow velocity
         ! we need gas pressure, density, and velocity
         ...
         !AS
         ! exploding wire span: EWleft, EWright
         ! High pressure: GP1, Grho1
         ! low pressure: GP0, Grho0

         call param_read('Exploding wire left',EWleft)
         call param_read('Exploding wire right',EWright)
         call param_read('Post-shock density',Grho1)
         call param_read('Post-shock pressure',GP1)
         ...
         if (amRoot) then
           ...
           print*, 'High pressure region spans from',EWleft,'to',EWright
         ...
         do i=fs%cfg%imino_,fs%cfg%imaxo_
            if ((fs%cfg%xm(i).gt.EWleft).and.(fs%cfg%xm(i).le.EWright))then ! high pressure region
               fs%Grho(i,:,:) = Grho1
               fs%Ui(i,:,:) = 0.0_WP
               fs%GP(i,:,:) = GP1
               fs%GrhoE(i,:,:) = matmod%EOS_energy(GP1,Grho1,0.0_WP,0.0_WP,0.0_WP,'gas')
            else ! low pressure region
               fs%Grho(i,:,:) = Grho0
               fs%Ui(i,:,:) = 0.0_WP
               fs%GP(i,:,:) = GP0
         ...
         !AS: ***Boundary conditions on left and right sides replaced with walls***
