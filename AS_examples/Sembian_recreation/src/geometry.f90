...
contains
   ...
   subroutine geometry_init
      ...
      create_grid: block
         ...
         real(WP) :: rdx,dx,box_x1,box_x2,dy,box_y1,ddrop
         real(WP), dimension(3) :: dctr
         real(WP) :: Lcalc,r,rold,err,tol
         real(WP) :: Lx,Ly,Lz
         real(WP), dimension(:), allocatable :: x,y,z

         ! AS variables for mesh stretching in x 
         real(WP), dimension(:), allocatable :: temp_x,x_full !AS temporary arrays
         real(WP) :: alpha,dx_old,dx_ref,start_stretch,dx_stretch !AS 
         integer :: nx_stretch,nx_full !AS

         ! Read in grid definition
         call param_read('Lx',Lx); call param_read('nx',nx); call param_read('nx stretch',nx_stretch); allocate(x(nx+nx_stretch+1)); call param_read('Lx stretch', start_stretch, default=0.0_WP) !AS
         dx = Lx/nx

         !allocate and initialize temporary arrays for stretching 
         allocate(temp_x(nx_stretch+1)); temp_x(:) = 0.0_WP
         allocate(x_full(nx+nx_stretch+1)); x_full(:) = 0.0_WP
         ...
         if (param_exists('Cells per diameter')) then
           ...
           if (amRoot) then
            ...
            !AS mesh generation
            dx_ref = (Lx - start_stretch)/real(nx,WP)
            alpha = 1.05 !stretching ratio in x 
            nx_full = nx_stretch + nx !total number of points in x

            !generate uniform refined region
            do i=1,nx+1
               x(i) = start_stretch + real(i-1,WP)*dx_ref
            end do
            
            !populate temporary full x array with uniform region
            do i=nx_full-nx,nx_full+1
               x_full(i) = x(i-nx_stretch)
            end do

            !generate stretched region ---> how can I rewrite this with an exponent instead of dx_stretch?
            do i=nx_stretch,1,-1
               dx_old = x_full(i+2) - x_full(i+1)
               dx_stretch = dx_old*alpha
               x_full(i) = x_full(i+1) - dx_stretch
            end do

            !AS replace x array with new mesh
            x(:) = x_full(:)
            ...
            if (amRoot) then
               ...
               print*, 'Start of refined region: ',Lx
               print*, 'Uniform spacing dx_ref: ',dx_ref
               print *, "Stretching ratio in x direction: ",alpha
               print*, "Length added to domain in x direction: ", x(nx) - x(1)
               print*, "Full domain length: ", x(nx_full + 1) - x(1)
               print*, '==== MESH DESCRIPTIN IN Y ===='
               print*, 'Uniform in Y (for now): ', Ly
               print*, 'Uniform spacing in Y: ',dy
               print*, 'Aspect ratio dx/dy: ',dx_ref/dy
            end if
      ...
      create_walls: block
        ...
        integer :: i, j, k, nx, ny, nz
        logical :: wall_left, wall_right, wall_top, wall_bottom !wall definition parameters in input file
        
        call param_read('wall left',wall_left)
        call param_read('wall right',wall_right)
        call param_read('wall top',wall_top)
        call param_read('wall bottom',wall_bottom)

        !right wall
        if (wall_right.eqv.(.true.)) then
           if (amRoot) then
              print *, "Right wall constructed."
           end if
           cfg%VF(cfg%imaxo_+1,:,:) = 0.0_WP
        end if

        !left wall
        if (wall_left.eqv.(.true.)) then
           if (amRoot) then
              print *, "Left wall constructed."
           end if

           cfg%VF(cfg%imino_-1,:,:) = 0.0_WP
        end if

        !top wall
        if (wall_top.eqv.(.true.)) then
           if (cfg%jproc.eq.cfg%npy) then
             cfg%VF(:,cfg%jmax+1,:)=0.0_WP !create walls on top of domain (in the ghost cells)
           end if

           if (amRoot) then
              print *, "Top wall constructed."
           end if
        end if

        !bottom wall (could be used to act as a symmetry BC if the simulation is inviscid and droplet is place properly
        if (wall_bottom.eqv.(.true.)) then
            if (cfg%jproc.eq.1) then
              cfg%VF(:,cfg%jmin-1,:) = 0.0_WP !create walls on bottom of domain (in the ghost cells)
            end if

            if (amRoot) then
              print*, "Bottom wall constructed."
            end if
        end if
