...
contains
   ...
   subroutine simulation_init
      ...
      create_and_initialize_flow_solver: block
         ...
         do i=fs%cfg%imino_,fs%cfg%imaxo_
           ! pressure, velocity, use matmod for energy
           if (fs%cfg%x(i).lt.interface_loc) then ! IC on left side of interface is working 10/16/2023
            fs%Grho(i,:,:) = 0.0_WP
            fs%Lrho(i,:,:) = Lrho0
            fs%Ui(i,:,:) = Lu0
            fs%GP(i,:,:) = 0.0_WP
            fs%GrhoE(i,:,:) = matmod%EOS_energy(GP0,0.0_WP,Gu0,0.0_WP,0.0_WP,'gas')
            fs%LrhoE(i,:,:) = matmod%EOS_energy(LP0,Lrho0,Lu0,0.0_WP,0.0_WP,'liquid')
           else
             fs%Grho(i,:,:) = Grho0
             fs%Lrho(i,:,:) = 0.0_WP
             fs%Ui(i,:,:) = Gu0
             fs%GP(i,:,:) = GP0 !AS gas phase pressure initializing with nans (I think this is a problem from dimensionalizing the sim, similar to shock_droplet example)
