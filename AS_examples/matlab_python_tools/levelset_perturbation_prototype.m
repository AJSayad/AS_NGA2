...
d0 = 0.00187 %1.0;  % diameter
r0 = d0/2; % radius
nx = 1000;
ny = 1000;
x = linspace(-2*d0,2*d0,nx);
y = linspace(-2*d0,2*d0,ny);
[XX, YY] = meshgrid(x,y);

%% geometry definition
epsilon = d0*[0.05, 0.0, 0.0]; % amplitude of our perturbation
mode = [2, 0, 0];                 % wave number modes for our perturbation (spatial frequency of wave, 2pi/lambda) lambda=linear wavelength
phase = [0,0,0];             % phase shifts (offset of waves)
