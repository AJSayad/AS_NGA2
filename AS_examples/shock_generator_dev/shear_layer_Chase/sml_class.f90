...
contains
   ...
   subroutine init(this)
      ...
      create_config: block
         ...
         grid=sgrid(coord=cartesian,no=3,x=x,y=y,z=z,xper=.true.,yper=.false.,zper=.true.,name='sml')
         ! Read in partition
      ...
      initialize_timetracker: block
         ...
         call param_read('Max timestep size sml',this%time%dtmax)
      ...
      real(WP) :: c,r_vel
         ...
         call param_read('Velocity ratio', r_vel)
         ...
         do j=this%fs%cfg%jmino_,this%fs%cfg%jmaxo_
            if (this%fs%cfg%ym(j).le.0.0_WP) then
               this%meanU(j)=r_vel*tanh(this%cfg%ym(j)/c) + r_vel
            else
               this%meanU(j)=(1.0_WP - r_vel)*tanh(this%fs%cfg%ym(j)/c) + r_vel
            end if
            ! this%meanU(j)=0.5_WP*tanh(this%cfg%ym(j)*0.5_WP)
         end do
         ! ! Initialize to tanh profile
         ! do j=this%fs%cfg%jmino_,this%fs%cfg%jmaxo_
      ...
      create_monitor: block
         ...
         this%mfile=monitor(this%fs%cfg%amRoot,'simulation_sml')
         ...
         ! Create CFL monitor
         this%cflfile=monitor(this%fs%cfg%amRoot,'cfl_sml')
