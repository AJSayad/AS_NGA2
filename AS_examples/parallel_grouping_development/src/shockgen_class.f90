...
contains
  ...
  subroutine init(this,shockgen_group)
    ...
    create_config: block
      ...
      ! set ny and nz to 1
      ny = 1; ny_stretch = 0; nz = 1; nz_stretch = 0
      ! set Ly and Lz to dx
      Ly = dx; Lz = dx
      allocate(x(nx+nx_stretchL+nx_stretchR+1));allocate(y(ny));allocate(z(nz));
  ...
  subroutine final(this,shockgen_group)
    ...
    print*, "shockgen final subroutine: shockgen_group = ", shockgen_group
    ...
    allocate(Grho_global(this%cfg%imin:this%cfg%imax));  Grho_global  = 0.0_WP
    allocate(GrhoE_global(this%cfg%imin:this%cfg%imax)); GrhoE_global = 0.0_WP
    allocate(GP_global(this%cfg%imin:this%cfg%imax));    GP_global    = 0.0_WP
    allocate(Ui_global(this%cfg%imin:this%cfg%imax));    Ui_global    = 0.0_WP
    
    ! allocate local center arrays
    allocate(Grho_center(this%cfg%imin:this%cfg%imax));  Grho_center  = 0.0_WP
    allocate(GrhoE_center(this%cfg%imin:this%cfg%imax)); GrhoE_center = 0.0_WP
    allocate(GP_center(this%cfg%imin:this%cfg%imax));    GP_center    = 0.0_WP
    allocate(Ui_center(this%cfg%imin:this%cfg%imax));    Ui_center    = 0.0_WP
