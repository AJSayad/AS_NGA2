...
contains
  ...
  subroutine final(this)
    ...
    allocate(Ui_center(this%cfg%nx_)); Ui_center = 0.0_WP

    ! store local centerline variables on each processor
    Grho_center  = this%fs%Grho(this%cfg%imin_:this%cfg%imax_,ny_center,nz_center)
    GrhoE_center = this%fs%GrhoE(this%cfg%imin_:this%cfg%imax_,ny_center,nz_center)
    GP_center    = this%fs%GP(this%cfg%imin_:this%cfg%imax_,ny_center,nz_center)
    Ui_center    = this%fs%Ui(this%cfg%imin_:this%cfg%imax_,ny_center,nz_center)

    ! add MPI calls here
    !-----------------------------------------------------------------------
    ! 3. Build recvcounts and displacements (displs)
    !    recvcounts(i) = number of elements to receive from rank i
    !    displs(i)     = starting index in the global array for rank i
    !-----------------------------------------------------------------------
    ...
    displs(1) = 0
    ! for each subsequent rank, the offset is the sum of all previous sizes
    do i = 2, this%cfg%nproc
       displs(i) = displs(i-1) + recvcounts(i-1)
    end do
    allocate(Grho_global(nx_total));allocate(GrhoE_global(nx_total));allocate(GP_global(nx_total));allocate(Ui_global(nx_total))

    !-----------------------------------------------------------------------
    ! 5. Perform the collective Allgatherv
    !    - sendbuf:   Grho_center, length = this%cfg%nx_
    !    - recvbuf:   Grho_global, with counts and displs per rank
    !-----------------------------------------------------------------------
    ! communicate centerline values accorss cores
    call MPI_Allgatherv( Grho_center,         & ! local send buffer
                     to_send,                 & ! number of elements I send
                     MPI_DOUBLE_PRECISION,             & ! my real(kind=WP) type
                     Grho_global,             & ! recv buffer (global)
                     recvcounts,              & ! how many to take from each rank
    ...
    call MPI_Allgatherv( GrhoE_center,        & ! local send buffer
                     to_send,                 & ! number of elements I send
                     MPI_DOUBLE_PRECISION,             & ! my real(kind=WP) type
                     GrhoE_global,            & ! recv buffer (global)
                     recvcounts,              & ! how many to take from each rank
                     displs,                  & ! where in Grho_global to place each block
                     MPI_DOUBLE_PRECISION,             & ! data type of recv buffer
                     MPI_COMM_WORLD, ierr )     ! communicator
    call MPI_Allgatherv( GP_center,           & ! local send buffer
                     to_send,                 & ! number of elements I send
                     MPI_DOUBLE_PRECISION,             & ! my real(kind=WP) type
                     GP_global,               & ! recv buffer (global)
                     recvcounts,              & ! how many to take from each rank
                     displs,                  & ! where in Grho_global to place each block
                     MPI_DOUBLE_PRECISION,             & ! data type of recv buffer
                     MPI_COMM_WORLD, ierr )     ! communicator
    call MPI_Allgatherv( Ui_center,           & ! local send buffer
                     to_send,                 & ! number of elements I send
                     MPI_DOUBLE_PRECISION,             & ! my real(kind=WP) type
                     Ui_global,               & ! recv buffer (global)
                     recvcounts,              & ! how many to take from each rank
                     displs,                  & ! where in Grho_global to place each block
                     MPI_DOUBLE_PRECISION,             & ! data type of recv buffer
                     MPI_COMM_WORLD, ierr )     ! communicator
