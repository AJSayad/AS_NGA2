...
module simulation
   ...
   !> AIAA WG parameters
   real(WP) :: M_inf,Re,We,rho_ratio,mu_ratio,c_ratio,tc
   real(WP) :: GP2,Grho2,u2,c2,GP1,Grho1,u1,M1,c1,Lrho1,LP1,Ms,delta_u,cL
   real(WP) :: A,B,C
...
contains
   ...
   subroutine simulation_init
      ...
      !!! AS: read in AIAA WG parameters here
      ...
      initialize_parameters: block
         ...
         PinfG=0.0_WP ! Set PinfG to zero 
         ! AS: read in AIAA WG parameters here
         call param_read('Shock generator' ,sgenflag) ! shock generator flag
         call param_read('M_inf', M_inf)
         call param_read('Re', Re)
         ! call param_read('We', We)
         call param_read('Density ratio', rho_ratio)
         call param_read('Viscosity ratio', mu_ratio)
         call param_read('Sound speed ratio', c_ratio)
         call param_read('Liquid gamma',GammaL)
         call param_read('Gas gamma',GammaG)
         call param_read('Drop location', dctr)
         call param_read('Shock location', Xshock)
         
         ! Following steps from AIAA WG normalization strategy
         Grho2  = 1.0_WP                  ! normalized gas density
         ddrop  = 1.0_WP                  ! normalized drop diameter
         GP2 = 1/(GammaG*M_inf**2)        ! post-shock gas pressure
         ...
         write(message,'("[AIAAWG] Shock Mach number           = ",es12.5)')            Ms   ; call log(message)
         write(message,'("[AIAAWG] Characteristic time scale   = ",es12.5)')            tc   ; call log(message)
         write(message,'("[AIAAWG] Liquid viscosity            = ",es12.5)')            muL  ; call log(message)
         write(message,'("[AIAAWG] Gas viscosity               = ",es12.5)')            muG  ; call log(message)
         ...
         write(message,'("[AIAAWG] Drop diameter (enforced)    = ",es12.5)')         ddrop   ; call log(message)
         write(message,'("[AIAAWG] +++++++ Post-shock Conditions +++++++")')                 ; call log(message)
         write(message,'("[AIAAWG] Pressure                    = ",es12.5)')            GP2  ; call log(message)
         write(message,'("[AIAAWG] Density (enforced)          = ",es12.5)')            Grho2; call log(message)
         write(message,'("[AIAAWG] Velocity (shock frame)      = ",es12.5)')            u2   ; call log(message)
         write(message,'("[AIAAWG] Delta_u(lab frame, enforced)= ",es12.5)')         delta_u ; call log(message)
         write(message,'("[AIAAWG] Mach number (M_inf)         = ",es12.5)')          M_inf  ; call log(message)
         write(message,'("[AIAAWG] Post-shock sound speed      = ",es12.5)')            c2   ; call log(message)
         write(message,'("[AIAAWG] ++++++++ Thermodynamics +++++++++++++")')                 ; call log(message)
         write(message,'("[AIAAWG] Liquid gamma                = ",es12.5)')          GammaL ; call log(message)
         write(message,'("[AIAAWG] Liquid stiffness pressure   = ",es12.5)')           PinfL ; call log(message)
         write(message,'("[AIAAWG] Liquid specific heat (CvL)  = ",es12.5)')             CvL ; call log(message)
         write(message,'("[AIAAWG] Gas gamma                   = ",es12.5)')          GammaG ; call log(message)
         write(message,'("[AIAAWG] Gas specific heat    (CvG)  = ",es12.5)')             CvG ; call log(message)
      ...
      !!! AS: set our initial conditions for the AIAA WG case here
         ...
         do k=cfg%kmino_,cfg%kmaxo_
            do j=cfg%jmino_,cfg%jmaxo_
               do i=cfg%imino_,cfg%imaxo_
                  ...
                  fs%U(i,j,k)=delta_u*Hshock(Xshock-fs%cfg%x(i),delta=0.5_WP*fs%dx)
                  fs%V(i,j,k)=0.0_WP; fs%W(i,j,k)=0.0_WP
                  if (fs%VF(i,j,k).lt.1.0_WP) then
                     fs%RHOG(i,j,k)=Grho1+(Grho2-Grho1)*Hshock(Xshock-fs%cfg%xm(i),delta=0.5_WP*fs%dx)
                     fs%PG  (i,j,k)=GP1  +(GP2  -GP1  )*Hshock(Xshock-fs%cfg%xm(i),delta=0.5_WP*fs%dx)
      ...
      create_ensight: block
         ...
         ushock = u1 ! set shock velocity from main sim
         if (sgenflag.eqv.(.true.))then
            ...
            Xend = ddrop + Xshock !> let the shock travel 1 diameter
            if(amRoot)then   !> setup and run sg only on root proc
               call sgen_init(dx,[nx,1,1],X0,sgen_group,muG,Xshock,Xend,ushock)
            ...
            shock_index = ceiling(abs(Xshock - cfg%xm(1))/cfg%dx(1))
   ...
   contains
      ...
      function levelset_drop(xyz,t) result(G)
         ...
         real(WP), intent(in) :: t
         real(WP) :: G
         G=1.0_WP-sqrt(((xyz(1)-dctr)/ddrop*2.0_WP)**2+(xyz(2)/ddrop*2.0_WP)**2+(xyz(3)/ddrop*2.0_WP)**2)
   ...
   subroutine sgen_init(dx,meshsize,X0,sgen_group,viscG,Xshock,Xend,ushock)
      use param,    only: param_read
      use parallel, only: group,amRoot
      use mpi_f08,  only: MPI_GROUP,MPI_COMM_WORLD,MPI_DOUBLE_PRECISION
      implicit none
      real(WP), intent(in)  :: dx,viscG,ushock,Xshock,Xend
      ...
      sg%time%tmax = (Xend - Xshock)/ushock ! this is how long it will take to reach Xend
      do k=sg%cfg%kmino_,sg%cfg%kmaxo_
         do j=sg%cfg%jmino_,sg%cfg%jmaxo_
            do i=sg%cfg%imino_,sg%cfg%imaxo_
               sg%fs%U(i,j,k)=delta_u*Hshock(Xshock-sg%fs%cfg%x(i),delta=0.5_WP*sg%fs%dx); sg%fs%V(i,j,k)=0.0_WP; sg%fs%W(i,j,k)=0.0_WP ! velocity
               sg%fs%Q(i,j,k,1)=Grho1+(Grho2-Grho1)*Hshock(Xshock-sg%fs%cfg%xm(i),delta=0.5_WP*sg%fs%dx)                              ! density
               sg%fs%P   (i,j,k)=GP1  +(GP2  -GP1 )*Hshock(Xshock-sg%fs%cfg%xm(i),delta=0.5_WP*sg%fs%dx)                             ! pressure
   ...
   subroutine update_shockprofile(rhog,pg,ig,u,rhog_profile,pg_profile,ig_profile,U_profile,IMIN,IMAX,JMIN,JMAX,KMIN,KMAX,nshock,shock_index)
      ...
      do i = IMIN,IMAX
         if (i.lt.shock_index) then
            rhog(i,:,:) = Grho2
            pg  (i,:,:) = GP2
            ig  (i,:,:) =(GP2 + GammaG*PinfG)/(Grho2*(GammaG-1.0_WP))
            u   (i,:,:) = delta_u
         else
            rhog(i,:,:) = Grho1
            pg  (i,:,:) = GP1
   ...
   subroutine apply_bconds()
      ...
      ! Apply Dirichlet BC on left boundary for constant inflow 
      if (.not.fs%cfg%xper.and.fs%cfg%iproc.eq.1) then
         do k=fs%cfg%kmino_,fs%cfg%kmaxo_; do j=fs%cfg%jmino_,fs%cfg%jmaxo_
            ! Set constant values
            do i=fs%cfg%imino,fs%cfg%imin
               ! Copy primitive variables
               fs%RHOL(i,j,k)=Lrho1
               fs%PL  (i,j,k)=LP1
               fs%IL  (i,j,k)=(LP1 + GammaL*PinfL)/(Lrho1*(GammaL-1.0_WP))
               fs%RHOG(i,j,k)=Grho2
               fs%PG  (i,j,k)=GP2
               fs%IG  (i,j,k)=(GP2 + GammaG*PinfG)/(Grho2*(GammaG-1.0_WP))
               !fs%U  (i,j,k)=max(fs%U(fs%cfg%imax,j,k),0.0_WP)
               fs%U   (i,j,k)=delta_u
               fs%V   (i,j,k)=0.0_WP
               fs%W   (i,j,k)=0.0_WP
