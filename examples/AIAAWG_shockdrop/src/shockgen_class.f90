...
module shockgen_class  
  ...
  type :: shockgen
   ...
   contains
      procedure :: initialize                      !< Initialize shock-generator simulation
      procedure :: step                            !< Advance shock-generator simulation by one time step
      procedure, private :: prepare_viscosities    !< Prepare viscosities for shock-generator simulation
      procedure :: finalize                        !< Finalize shock-generator simulation
      procedure :: output_monitor                  !< Monitoring for shock-generator case
...
contains
  ...
  subroutine initialize(this,dx,meshsize,startloc,sgen_group,partition)
    use param,    only: param_read
    use mpi_f08, only: MPI_Group
    implicit none
    class(shockgen), intent(inout) :: this
    real(WP), intent(in) :: dx
    integer , dimension(3)  :: meshsize
    ...
    create_config: block
      ...
      ! By default, the shock generator is fully periodic
      xper=.false.; yper=.true.; zper=.true.
      meshsize(2) = 1; meshsize(3) = 1 ! set to a 1D case 
      ! Create simple rectilinear grid
      allocate(x(meshsize(1)+1)); do i=1,meshsize(1)+1; x(i)=real(i-1,WP)*dx+startloc(1); end do
      allocate(y(meshsize(2)+1)); y(1) = 0.0_WP; y(2) = dx
      allocate(z(meshsize(3)+1)); z(1) = 0.0_WP; z(2) = dx
      ! General serial grid object
      grid=sgrid(coord=cartesian,no=3,x=x,y=y,z=z,xper=xper,yper=yper,zper=zper,name='ShockGen')
      ! Deallocate z/y/z 
      deallocate(x,y,z)
      ! Create config
      this%cfg=config(grp=sgen_group,decomp=partition,grid=grid)      
    end block create_config

    ! Initialize time tracker
    initialize_timetracker: block
      this%time=timetracker(amRoot=this%cfg%amRoot,name='ShockGen')
    end block initialize_timetracker

    ! Create singlephase compressible flow solver
  ...
  subroutine finalize(this,nshock,Xend,RHOG_profile,IG_profile,PG_profile,U_profile)
      ...
      ! assume ddrop = 1 and that the shock travles 1 diameter, the final shock location is passed as an argument from sim.f90
